...
###############################################################################
# PVT_embed remains if you want pvt. Otherwise skip.
###############################################################################
...
###############################################################################
# A CLIP-based "image" embedder for patches. We do upsample from 4×4 → 224×224.
###############################################################################
CLIP_PATH = "home/ij_students/sonu/checkpoints-13675"  # change if saved elsewhere

class CLIPPatchEmbed(nn.Module):
    """Flattened 3×4×4 patch → 512‑d CLIP image feature."""
    def __init__(self, freeze=False):
        ...
        self.proj = nn.Linear(512, 128)  # will adjust to n_hidden later
    def forward(self, patches):                    # (B_p,T,48)
        Bp,T,_ = patches.shape; dev = patches.device
        imgs = patches.reshape(Bp*T,3,4,4)
        imgs = F.interpolate(imgs,(224,224),mode='bilinear',align_corners=False)
        inp  = self.proc(images=imgs,return_tensors='pt').to(dev)
        with torch.set_grad_enabled(self.proj.weight.requires_grad):
            feat = self.clip.get_image_features(**inp)   # (Bp*T,512)
        feat = feat.view(Bp,T,512)
        return self.proj(feat)               # → (Bp,T,hidden)

class CLIPTextEncoder(nn.Module):
    def __init__(self, freeze=False):
        super().__init__(); self.clip = CLIPModel.from_pretrained(CLIP_PATH); self.proc = CLIPProcessor.from_pretrained(CLIP_PATH)
        if freeze:
            for p in self.clip.parameters(): p.requires_grad = False
    def forward(self,caps:List[str],device):
        inp = self.proc(text=caps,return_tensors='pt',padding=True,truncation=True).to(device)
        return self.clip.get_text_features(**inp)  # (B,512)
...
class ContextFormer(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        # We'll keep your text-based CLIP for weather/caption if needed:
        self.clip_encoder = CLIPSimpleEncoder(freeze=True)
        self.text_proj = nn.Linear(512, hparams.n_weather)

        # For images: if pvt, use PVT_embed; else, use our new CLIPPatchEmbed:
        if self.hparams.pvt:
            ...
            # NEW: Use our patch-based CLIP
            self.embed_images = CLIPPatchEmbed()
    ...
    def add_model_specific_args(
        ...
        parser.add_argument("--patch_size", type=int, default=4)  # 4x4 patch
        parser.add_argument("--n_image", type=int, default=3)     # 3 channels for CLIP
    ...
    def forward(self, data, pred_start: int = 0, preds_length: Optional[int] = None):
        ...
        # ---------------------------------------------------------
        # 1) bookkeeping: padding, shuffle (unchanged)
        # ---------------------------------------------------------
        ...
        if T == c_l and preds_length:
            pad = (torch.zeros(B, preds_length, C, H, W, device=hr_dynamic_inputs.device),
                torch.zeros(B, preds_length, 1, H, W, device=hr_dynamic_mask.device))
            hr_dynamic_inputs = torch.cat([hr_dynamic_inputs, pad[0]], 1)
            hr_dynamic_mask   = torch.cat([hr_dynamic_mask,   pad[1]], 1)
            B, T, C, H, W = hr_dynamic_inputs.shape                     # refresh

        static_inputs = data["static"][0][:, :3]                        # (B,3,H,W)

        if self.hparams.spatial_shuffle:
            perm    = torch.randperm(B * H * W, device=hr_dynamic_inputs.device)
            invperm = inverse_permutation(perm)
            hr_dynamic_inputs = hr_dynamic_inputs.permute(1,2,0,3,4)\
                                .reshape(T, C, -1)[:, :, perm]\
                                .reshape(T, C, B, H, W).permute(2,0,1,3,4)
            static_inputs = static_inputs.permute(1,0,2,3).reshape(3, -1)[:, perm]\
                            .reshape(3,B,H,W).permute(1,0,2,3)
        else:
            invperm = None                                             # defined for later

        # ---------------------------------------------------------
        # 2) fuse weather  +  caption embeddings (fine‑tuned CLIP)
        # ---------------------------------------------------------
        weather            = data["dynamic"][1]                        # (B,T,Cw)
        _, t_m, c_m        = weather.shape
        captions           = data.get("caption", [""] * B)

        device = hr_dynamic_inputs.device
        with torch.no_grad():
            txt_clip   = self.clip_encoder(captions, device=device)    # (B,512)
        txt_proj          = self.text_proj(txt_clip)                   # (B,n_weather)

        text_rep          = txt_proj.unsqueeze(1).repeat(1, T, 1)      # (B,T,n_weather)
        fused_weather     = torch.cat([weather, text_rep], -1)         # (B,T, 24+24)

        fuse_mlp          = Mlp(fused_weather.size(-1),
                                self.hparams.n_hidden,
                                self.hparams.n_weather).to(device)
        with torch.no_grad():
            weather = fuse_mlp(fused_weather)                          # (B,T, 24)

        # ---------------------------------------------------------
        # 3) prepare image tensor = [dynamic  +  static]
        # ---------------------------------------------------------
        static_inputs = static_inputs.unsqueeze(1).repeat(1, T, 1, 1, 1)
        images        = torch.cat([hr_dynamic_inputs, static_inputs], 2)  # (B,T,3,H,W)
        B, T, _, H, W = images.shape

        # ---------------------------------------------------------
        # 4) patchify  +  CLIP image embedding  (no PVT branch)
        # ---------------------------------------------------------
        ps        = self.hparams.patch_size          # 4
        ph, pw    = H // ps, W // ps
        patches_per_img = ph * pw                    # e.g. 16×16 = 256

        image_patches = images.reshape(
                B, T, 3, ph, ps, pw, ps)             \
                .permute(0,3,5,1,2,4,6)              \
                .reshape(B * patches_per_img, T, 3*ps*ps)   # (B_patch, T, 48)

        B_patch, N_patch, _ = image_patches.shape    # N_patch = T
        image_patches_embed = self.embed_images(image_patches)         # (B_patch,T,n_hidden)

        # ---------------------------------------------------------
        # 5) masks & weather‑per‑patch
        # ---------------------------------------------------------
        mask_patches = hr_dynamic_mask.reshape(
                B, T, 1, ph, ps, pw, ps)             \
                .permute(0,3,5,1,2,4,6)              \
                .reshape(B_patch, T, ps*ps)          # (B_patch,T,16)

        weather_patches = weather.unsqueeze(1) \
                .repeat(1, patches_per_img, 1, 1)    \
                .reshape(B_patch, T, c_m)            # (B_patch,T,24)

        weather_patches_embed = self.embed_weather(weather_patches)    # (B_patch,T,n_hidden)


        # The same token mask logic as before
        if (
            self.hparams.mtm
            and self.training
        ...
        x_out[
         ...
         # unpatchify => images_out shape [B, N_patch, n_out, H, W]
        images_out = (
            x_out.reshape(
                B,
