...
class EarthNet2021XOldDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        try:
            ...
            print(f"[WARNING] Skipping file due to general read error: {filepath.name}\n{e}")
            return self.__getitem__((idx + 1) % len(self))

        # Try reading llama_caption
        llama_caption = None
        if "llama_caption" in minicube.variables:
            try:
                llama_caption = minicube.llama_caption.values
            except Exception as e:
                print(f"[INFO] Failed to read 'llama_caption' in {filepath.name}, skipping this field.\n{e}")
                llama_caption = None
        ...
        if llama_caption is not None:
            data["llama_caption"] = torch.from_numpy(llama_caption.astype(self.type))
        return data
...
class EarthNet2021XOldDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        if stage == 'fit' or stage is None:
            earthnet_corpus = EarthNet2021XOldDataset(self.base_dir/"train", fp16 = self.hparams.fp16, spatial_eobs = self.hparams.spatial_eobs, eobs_spread = self.hparams.eobs_spread, soilgrids_all = self.hparams.soilgrids_all)
        ...
        if stage == 'test' or stage is None:
            self.earthnet_test = EarthNet2021XOldDataset(self.base_dir/"test"/self.hparams.test_track, fp16 = self.hparams.fp16, spatial_eobs = self.hparams.spatial_eobs, eobs_spread = self.hparams.eobs_spread, soilgrids_all = self.hparams.soilgrids_all)
...
class EarthNet2021XpxOldDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if stage == 'fit' or stage is None:
            ...
            self.earthnet_train = EarthNet2021XpxOldDataset(self.base_dir, fp16 = self.hparams.fp16, eobs_spread = self.hparams.eobs_spread, soilgrids_all = self.hparams.soilgrids_all)
